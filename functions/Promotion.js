...
asyncRouter.post("/create", async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).send({ result: "Create promotion success" });
  } catch (err) {
    console.log(err);
    return next(ERRORS.DATA.INVALID_DATA);
  }
...
asyncRouter.post("/delete", async (req, res, next) => {
  ...
  try {
    await DB.promotion.doc(body.docId).delete();
    res.status(200).send({ result: "Delete promotion success" });
  } catch (err) {
    console.log(err);
    return next(ERRORS.DATA.INVALID_DATA);
  }
